...
class M_Hotel_Bookings
{
    ...
    public function viewbookings($usertype, $userid)
    {
        if ($usertype == 'Traveler') {
            ...
            $this->db->query('SELECT * FROM hotel_bookings where hotel_id=:userID');
            $this->db->bind(':userID', $userid);
    ...
    public function deleteReviews($travelerID){
        $this->db->query('DELETE FROM hotel_reviews WHERE TravelerID=:travelerID and HotelID=:hotelID');

        $this->db->bind(':travelerID', $travelerID);
        $this->db->bind(':hotelID', $_SESSION['user_id']);
